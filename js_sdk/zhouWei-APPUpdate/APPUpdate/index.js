...
// 主颜色
const $mainColor = "#00919f";
// 弹窗图标url
const $iconUrl = "http://www.jnwater.com.cn/statics/jnwater/968133.png";
// http://www.jnwater.com.cn/statics/jnwater/968133.png
// 获取当前应用的版本号
...
const getServerNo = function(version,isPrompt = false, callback) {
	...
	/****************以下是示例*******************/
	// 可以用自己项目的请求方法
	$http.get("api/app/getNewApp", httpData,{
		isPrompt: isPrompt
	}).then(res => {
		/* res的数据说明
		 * | 参数名称	     | 一定返回 	| 类型	    | 描述
		...
		let versionInfo;
		if (platform == "android") {
			downloadurl = res.appLink
			versionCode = res.appCode
			versionName = res.appver
			versionInfo = res.appTips
		} else {
			downloadurl = res.iosLink
			versionCode = res.iosCode
			versionName = res.iosver
			versionInfo = res.iosTips
		}
		
		if (Number(version.versionCode) < Number(versionCode)) {
			callback && callback({
